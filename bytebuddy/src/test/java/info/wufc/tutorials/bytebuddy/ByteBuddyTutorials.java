...
public class ByteBuddyTutorials {
    ...
    @Test
    public void redefiningClass() throws IOException {
        // 利用了jvm的HotSwap特性 （热插拔）
        ByteBuddyAgent.install();
        Foo oldFoo = new Foo();
        DynamicType.Unloaded<Foo> make = new ByteBuddy().redefine(Foo.class).method(ElementMatchers.named("bar"))
                .intercept(FixedValue.value("foo"))
                .make();
        make.saveIn(new File(this.getClass().getClassLoader().getResource("").getPath() +"/debug"));
    ...
    @Test
    public void redefiningClass3() throws IOException {
        // 利用了jvm的HotSwap特性 （热插拔）
        ByteBuddyAgent.install();
        Foo oldFoo = new Foo();
        DynamicType.Unloaded<Foo> make = new ByteBuddy().redefine(Foo.class).method(ElementMatchers.named("bar"))
                .intercept(MethodDelegation.to(new Fin()))
                .make();
        make.saveIn(new File(this.getClass().getClassLoader().getResource("").getPath() +"/debug"));
        make.load(Foo.class.getClassLoader(), ClassReloadingStrategy.fromInstalledAgent());
        assertEquals(new Foo().bar(), "foo");
