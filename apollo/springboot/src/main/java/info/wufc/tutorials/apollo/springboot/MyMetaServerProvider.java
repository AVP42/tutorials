...
{
    private static final Logger LOG = LoggerFactory.getLogger(MyMetaServerProvider.class);
    ...
    public String getMetaServerAddress(Env targetEnv)
    {
        ...
        if (Strings.isNullOrEmpty(metaServerAddress)) {
            LOG.warn("Could not find meta server address, because it is not available in neither (1) JVM system property 'apollo.meta', (2) OS env variable 'APOLLO_META' (3) property 'apollo.meta' from server.properties nor (4) property 'apollo.meta' from app.properties (5) properties from apollo-metas.properties");
          } else {
              metaServerAddress = metaServerAddress.trim();
            LOG.info("Located meta services from apollo-metas configuration: {}!", metaServerAddress);
          }
